...
const render = () => {
  ...
  // Header
  document.getElementById('game-title').textContent = `${gameState.type} • Leg ${gameState.currentLeg}/${gameState.bestOf}`;
  document.getElementById('game-subtitle').textContent = `Best of ${gameState.bestOf}`;
...
const renderScoreboard = () => {
  ...
  scoreboard.innerHTML = gameState.players.map((player, idx) => {
    ...
    return `
      <div class="bg-white rounded-lg shadow-lg p-4 ${isCurrentPlayer ? 'ring-4 ring-green-500' : ''}">
        <div class="flex justify-between items-start mb-2">
          <div>
            <h2 class="text-xl font-bold ${isCurrentPlayer ? 'text-green-600' : 'text-gray-800'}">
              ${isCurrentPlayer ? '⭐ ' : ''}${player.name}
            </h2>
            <div class="text-sm text-gray-600">Legs won: ${player.legsWon}</div>
          </div>
          <div class="text-right">
            <div class="text-3xl font-bold text-purple-600">${player.currentScore}</div>
  ...
  panel.innerHTML = `
    <div class="bg-gradient-to-r from-green-500 to-green-600 rounded-lg shadow-lg p-4 text-white">
      ...
      <div class="bg-white/20 rounded p-2 mb-3 text-center">
        <div class="text-sm">Current turn: <span class="font-bold">${currentPlayer.name}</span></div>
        <div class="text-2xl font-bold mt-1">${gameState.currentTurn.join(', ') || '—'}</div>
      </div>
...
const setupAdminHandlers = () => {
  ...
  document.querySelectorAll('.score-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const score = btn.dataset.score === 'null' ? null : parseInt(btn.dataset.score);
      const multiplier = btn.dataset.multiplier ? parseInt(btn.dataset.multiplier) : selectedMultiplier;
...
document.getElementById('copy-link-btn').addEventListener('click', async () => {
  const link = isAdmin ? window.location.href : window.location.origin + window.location.pathname + '?id=' + gameId;
