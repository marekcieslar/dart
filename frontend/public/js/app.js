...
const loadGames = async (status, page, containerId, paginationId) => {
  ...
  try {
    ...
    container.innerHTML = data.games.map(game => `
      <a href="game.html?id=${game.id}" class="block p-4 border rounded-lg hover:bg-purple-50 hover:border-purple-300 transition">
        <div class="flex justify-between items-center">
          <div>
            <div class="font-bold text-lg">${game.type} • ${game.players.join(' vs ')}</div>
            <div class="text-sm text-gray-600">Score: ${game.currentScore}</div>
            <div class="text-xs text-gray-500">${formatDate(game.createdAt)}</div>
          </div>
    ...
    if (data.totalPages > 1) {
      ...
      paginationContainer.innerHTML = Array.from({ length: data.totalPages }, (_, i) => i + 1)
        .map(p => `
          <button 
            class="px-3 py-1 rounded ${p === page ? 'bg-purple-600 text-white' : 'bg-gray-200 hover:bg-gray-300'}"
    ...
    container.innerHTML = '<div class="text-center text-red-500 py-8">Error loading games</div>';
  }
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  loadGames('finished', finishedGamesPage, 'finished-games', 'finished-pagination');
});
