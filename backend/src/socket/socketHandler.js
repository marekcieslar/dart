  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join:game', async ({ gameId, adminToken }) => {
      try {
        ...
        console.log(`Client ${socket.id} joined game ${gameId} (admin: ${isAdmin})`);
      } catch (error) {
    ...
    // Add dart (admin only)
    socket.on('game:add-dart', async ({ gameId, adminToken, score, multiplier }) => {
      try {
        // Verify admin
        const { valid } = await verifyAdminToken(gameId, adminToken);
        if (!valid) {
        ...
        // Check for bust
        const completedTurn = result.currentTurn.length === 0 || result.currentTurn.length === 3;
